...
class SVGD(object):
    ...
    def _get_svgd_kernel(self, flatvars_list):
        ...
        lower = tfp.stats.percentile(pairwise_dists, 50.0, interpolation='lower')
        higher = tfp.stats.percentile(pairwise_dists, 50.0, interpolation='higher')
    ...
    def flatten_grads_and_vars(self, grads, vars):
        ...
        flatgrads =  tf.concat(axis=0, values=[
            tf.reshape(grad if grad is not None else tf.zeros_like(var), [self.get_num_elements(var)])
